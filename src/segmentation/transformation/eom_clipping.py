"""
EOM clipping transformation functions
Clips small EOM amounts to zero to reduce noise in the data
"""
...
def apply_eom_clipping(df: DataFrame, daily_amount_clip_threshold: float) -> DataFrame:
    ...
    # Clip values where absolute value is non-zero but below threshold
    df = df.with_columns(
        [
            "is_eom_clipped",
            "clipped_eom_amount",
        ...
        [
            F.when((F.abs(F.col("eom_amount")) < threshold), 1).otherwise(0),
            F.when((F.abs(F.col("eom_amount")) < threshold), F.col("eom_amount")).otherwise(0),
        ],
    )

    # Apply clipping to EOM amount only
    # Clip to zero if non-zero and absolute value is below threshold
    df = df.with_column(
        "eom_amount",
        F.when((F.abs(F.col("eom_amount")) < threshold), 0).otherwise(F.col("eom_amount")),
    )
