...
class DirectionRandomForestModel(BaseForecastModel):
    """Direction-aware Random Forest model that performs data transformations before fitting."""

    name: Literal["direction_random_forest"] = Field(description="Name of the model", default="direction_random_forest")
    ...
    models: dict[str, RandomForestRegressor] | None = Field(
        description="Fitted Random Forest models for each direction", default=None
    )
    ...
    def fit(
        ...
        def _fit_direction(direction_data):
            """Fit Random Forest for a specific direction"""
    ...
    def predict(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.models:
            raise ValueError(f"Direction Random Forest model must be fitted before prediction")
